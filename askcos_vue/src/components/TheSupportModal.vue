<template>
    <v-dialog v-model="dialogVisible" max-width="600px">
        <v-card>
            ...
            <v-card-text>
                <v-form>
                    <v-select v-model="selectedModule" :items="moduleOptions" :item-text="item => item.text" :item-value="item => item.value" label="Select module" ></v-select>
                    <v-select v-model="selectedCategory" :items="categoryOptions" item-text="text" item-value="value"
                        label="Select issue category"></v-select>
