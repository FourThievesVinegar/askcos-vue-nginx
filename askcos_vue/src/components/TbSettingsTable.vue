<template>
  <div id="settings">
    <div id="target" class="my-3">
      <CopyTooltip class="text-center" :data="settings.smiles">
        Target: <span class="smiles">{{ settings.smiles }}</span>
      </CopyTooltip>
      <div class="text-center">
        <SmilesImage v-if="settings.smiles" :smiles="settings.smiles"></SmilesImage>
      </div>
    </div>
    <table class="table mb-0">
      ...
      <tr>
        ...
        <td colspan=2>{{ tbVersion > 1000 ? `C++ v${tbVersion % 1000}` : `v${tbVersion}` }}</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan=2>{{ buyablesSources }}</td>
      </tr>
...
export default defineComponent({
  ...
  setup(props) {
    const tbVersion = computed(() => {
      // If version is not present in the result, then it is version 1
      return props.settings['version'] || 1;
    });

    const buyablesSources = computed(() => {
      if (props.settings['buyables_source']) {
        return sourceArgsToDisplay(sourceQueryToArgs(props.settings.buyables_source));
      } else {
        return 'All';
      }
    });
