...
describe("ASKCOS Homepage", () => {
  ...
  //   cy.get('[placeholder="SMILES"]').type("C1=CC(=C(C=C1F)F)C(CN2C=NC=N2)(CN3C=NC=N3)O");
  //   cy.get('[data-cy="home-canonicalize"]').invoke('removeAttr', 'target').click()
  // });
  ...
  it("Test Resolver", function () {
    ...
    // cy.get('[placeholder="SMILES"]').should( // doesn't work
    //   "have.text",
  ...
  it("Test Tree Builder", function () {
    ...
    cy.intercept("GET", "/api/legacy/celery/task/*").as("tree-task");
    cy.wait("@tree-task")
  });
  ...
  it("Test Predict Forward Synthesis", function () {
    cy.get('[placeholder="SMILES"]').type("C1CCC(OC2CCCCC2)CC1");
    cy.get('[data-cy="home-predict-impurties"]').invoke('removeAttr', 'target').click()
  ...
  it("Test Predict Aromatic Site Selectivity", function () {
    cy.get('[placeholder="SMILES"]').type("C1CCC(OC2CCCCC2)CC1");
    cy.get('[data-cy="home-site-selectivity"]').invoke('removeAttr', 'target').click()
  ...
  it("Test Solvent Screen", function () {
    cy.get('[placeholder="SMILES"]').type("C1CCC(OC2CCCCC2)CC1");
    cy.get('[data-cy="home-solvent-screen"]').invoke('removeAttr', 'target').click()
  ...
  it("Test QM Prediction", function () {
    cy.get('[placeholder="SMILES"]').type("C1CCCCCCC1");
    cy.get('[data-cy="home-qm-descriptors"]').invoke('removeAttr', 'target').click();
